...
export default function Dashboard() {
  ...
  const getUser = async () => {
    ...
    try {
      ...
      const request = await axios.get("http://localhost:3001/faculty/me", {
        headers: {
  ...
  const getClasses = async () => {
    try {
      ...
      const request = await axios.get(
        "http://localhost:3001/faculty/classrooms",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      console.log(request);
      setFacultyClasses(request.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleCreateClass = async (e: any) => {
    ...
    try {
      ...
      const request = await axios.post(
        "http://localhost:3001/faculty/create",
        newClassData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
                    ...
                    <li className="p-6">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          Semester {cls.semester} | Batches
                          {cls.batches.map((batch, idx) => (
                            <span key={idx}> {batch.batchId} </span>
                          ))}
                        </p>
                      </div>
                    </li>
